# GitHub Copilot HCD Customization Template
...
│   ├── prompts/                     # 25種類の明示実行Prompt
│   └── skills/                      # 3種類の再利用可能な専門Skill
├── .vscode/
│   └── settings.json                # Prompt/Instructionを有効化
├── docs/
...
- [Agent・Skill・Promptカスタマイズガイド](docs/reference/customizations.md)
...
- Skill: 3種類
- Prompt: 25種類
- Path-specific Instructions: 4種類
- HCD・UX成果物テンプレート: 8種類
- 依存パッケージ不要の検証スクリプト
...
- 複数Agent・Promptで再利用する手順は`.github/skills/`へ追加します。
- 同じルールを複数箇所へ重複させず、参照先を一つに保ちます。

## 注意

- Prompt filesは主に対応IDEから明示実行する機能です。
- AgentやSkillの利用可否は、Copilotプラン、組織ポリシー、IDEのバージョンにより異なります。
- Agentの`tools`は最小権限を基本とし、レビュー担当には編集権限を与えていません。
- テンプレート内の`[TODO: ...]`は導入先の情報に置き換えてください。
